...
from app.services.Prueba import PruebaService as Service

from app.models.Prueba import PruebaCreate,PruebaRead

from app.models.VotingType import VotingTypeCreate, VotingType
...
@prueba_router.post('/create', tags=tags, response_model=dict, status_code=201)
def create(prueba: PruebaCreate) -> dict:
    db = Session
    Service(db).create(prueba)
    return JSONResponse(status_code=201, content={'message': "se registro el nuevo estado"})

@prueba_router.get('/all', tags=tags, response_model=list[PruebaRead], status_code=200)
def get_all() -> dict:
    db = Session
...
def init()-> dict:
    ...
    voting = VotingCreate(maxVotes=3, status_id="clxldp91100073lpc4irqc48o", type_id="clxldp8xn00043lpcnl1duhim")

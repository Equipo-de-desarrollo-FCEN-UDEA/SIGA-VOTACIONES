...
def create(voting: VotingCreate, db: Session = Depends(get_session)) -> dict:
...
def get_voting_info(voting_id: str, db: Session = Depends(get_session)) -> VotingReadWithInfo:
    voting_db = Service(db).get_by_id(voting_id)
    return (voting_db)

@voting_router.post("/votings/{voting_id}/close", response_model=dict)
def close_voting(voting_id: str, db: Session = Depends(get_session)) -> dict:

...
from app.config.database import get_sesion
...
def create(status: StatusCreate, db: Session = Depends(get_sesion)) -> dict:
...
async def get_all(db: Session = Depends(get_sesion)) -> dict:
...
def get_status(status_id: str, db: Session = Depends(get_sesion)) -> dict:
